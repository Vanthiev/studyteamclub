...
outer:do{prevDiff=diff
prevPos=pos
var i=0
for(;i<steps;++i){var prevPos$1=pos
pos=moveVisually(cm,lineObj,pos,dir)
if(pos==null||pos.ch<begin||end<=(pos.sticky=="before"?pos.ch-1:pos.ch)){pos=prevPos$1
break outer}}
diff=cursorCoords(cm,pos,"line",lineObj,preparedMeasure).left-x
if(steps>1){var diff_change_per_step=Math.abs(diff-prevDiff)/steps
steps=Math.min(steps,Math.ceil(Math.abs(diff)/diff_change_per_step))
dir=diff<0?1:-1}}while(diff!=0&&(steps>1||((dir<0)!=(diff<0)&&(Math.abs(diff)<=Math.abs(prevDiff)))))
...
var el=document.createElement("div");el.style.cssText="\
    ...
    ";switch(style){case"default":el.style.background="white";break;case"success":el.style.background="skyblue";break;case"warning":el.style.background="orange";break;case"error":el.style.background="red";break;}
el.innerHTML=word;document.body.appendChild(el);setTimeout(function(){el.remove();},second*1000);}
function ajax(setting){setting.method=setting.method.toUpperCase();if(setting.method=="GET"&&setting.data)
setting.url+=(setting.data.indexOf("?")<0?"?":"&")+setting.data.serialize();var xhttp=new XMLHttpRequest();xhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){if(setting.function)setting.function(this.responseText);}};xhttp.open(setting.method,setting.url,true);if(setting.method=="POST"){xhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xhttp.send(setting.data);}else
xhttp.send();}
function post(url,data){ajax({url:url,method:"post",data:data,function:function(resp){message(JSON.parse(resp))}});}
Object.defineProperties(Object.prototype,{"serialize":{value:function(){var p,q=[];for(p in this)
if(this.hasOwnProperty(p))
q.push(encodeURIComponent(p)+"="+encodeURIComponent(this[p]));return q.join("&");}}});Object.defineProperties(Array.prototype,{"lastItem":{get:function(){return this[this.length-1];}}});Object.defineProperties(HTMLFormElement.prototype,{"serialize":{value:function(changedOnly=false){var i,j,q=[];for(i=this.elements.length-1;i>=0;i=i-1){if(this.elements[i].name===""||(changedOnly&&!this.elements[i].isChanged()))
continue;switch(this.elements[i].nodeName){case'INPUT':switch(this.elements[i].type){default:q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].value));break;case'checkbox':case'radio':if(this.elements[i].checked){q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].value));}
break;case'file':break;}
break;case'TEXTAREA':q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].value));break;case'SELECT':switch(this.elements[i].type){case'select-one':q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].value));break;case'select-multiple':for(j=this.elements[i].options.length-1;j>=0;j=j-1){if(this.elements[i].options[j].selected){q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].options[j].value));}}
break;}
break;case'BUTTON':switch(this.elements[i].type){case'reset':case'submit':case'button':q.push(this.elements[i].name+"="+encodeURIComponent(this.elements[i].value));break;}
break;}}
return q.join("&");}},"resetDefault":{value:function(){var i,j;for(i=this.elements.length-1;i>=0;i=i-1){if(this.elements[i].name==="")continue;switch(this.elements[i].nodeName){case'INPUT':switch(this.elements[i].type){default:this.elements[i].defaultValue=this.elements[i].value;break;case'checkbox':case'radio':this.elements[i].defaultChecked=this.elements[i].checked;break;case'file':case'password':break;}
break;case'TEXTAREA':this.elements[i].defaultValue=this.elements[i].value;break;case'SELECT':for(j=this.elements[i].options.length-1;j>=0;j--){if(this.elements[i].options[j].selected){this.elements[i].options[j].defaultSelected=this.elements[i].option[j].selected;}}
break;case'BUTTON':break;}}}},"send":{value:function(changedOnly=false,callback){ajax({url:this.action,method:this.method,data:this.serialize(changedOnly),function:function(resp){message(JSON.parse(resp));if(callback)callback(resp);}});}}});Object.defineProperties(HTMLTextAreaElement.prototype,{"isChanged":{value:function(){return this.defaultValue!=this.value;}}});Object.defineProperties(HTMLOptionElement.prototype,{"isChanged":{value:function(){return this.defaultSelected!=this.selected;}}})
Object.defineProperties(HTMLSelectElement.prototype,{"isChanged":{value:function(){var i;for(i=this.options.length-1;i>=0;i--){if(this.options[i].isChanged())return true;}
return false;}}});Object.defineProperties(HTMLInputElement.prototype,{"isChanged":{value:function(){switch(this.type){default:return this.defaultValue!=this.value;case"checkbox":case"radio":return this.defaultChecked!=this.checked;case"file":break;}}}});Object.defineProperties(Number.prototype,{"toBase":{value:function(base){var all="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";if(this<base)return all[this];else return parseInt(this/base).toBase(base)+all[this%base];}}});Object.defineProperties(String.prototype,{"reverse":{value:function(){return this.split("").reverse().join("");}},"toDecimal":{value:function(base){var all="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";var string=this.reverse();var result=0;for(var i=0,n=string.length;i<n;i++)
result+=all.indexOf(string[i])*Math.pow(base,i);return result;}}});var shareOn={facebook:function(url=location.href){window.open('https://www.facebook.com/sharer.php?u='+url,'sharer','toolbar=0,status=0,width=500,height=600');},googlePlus:function(url=location.href){window.open('https://plus.google.com/share?url='+url,'sharer','toolbar=0,status=0,width=500,height=600');},twitter:function(url=location.href){window.open('https://twitter.com/intent/tweet?url='+url,'sharer','toolbar=0,status=0,width=500,height=600');}};